...
)
{
	...
	if (state == 'r') myAction = riskState(stench, breeze, glitter, bump, scream);

	if (state == 'e') myAction = explorationState(stench, breeze, glitter, bump, scream);

	if (state == 'x') myAction = returnState(stench, breeze, glitter, bump, scream);
...
Agent::Action MyAI::explorationState(
	...
	*/
	...
	if (pathDestination == nullptr) {
		std::list<TileNode*> prunedFrontierNodes;
		for (TileNode* node : newFrontierNodes) {
			...
			if (node->stenchCount > 0 && monsterNode == nullptr && !killedMonster) isDangerous = true;
			if (!killedMonster && monsterNode != nullptr) {
				if (monsterNode == node) isDangerous = true;
				if (monsterNode->x == node->x && monsterNode->y == node->y && monsterNode != node) {
					isDangerous = true;
				}
			}
			...
			prunedFrontierNodes.push_back(node);
		}
		newFrontierNodes.clear();
		newFrontierNodes = prunedFrontierNodes;
		...
		if (dest != nullptr) {
			...
			/*
			if(monsterNode != nullptr) std::cout << "MONSTER?" << monsterNode->x << " " << monsterNode->y << " " << monsterNode << std::endl;
			std::cout << "CHOSE DESTINATION: " << dest->x << " " << dest->y << dest << std::endl;
			*/
		}
		else {
...
Agent::Action MyAI::riskState(
	...
	if (newFrontierNodes.size() > 0) {
		std::cout << "FOUND A PLACE TO GO! " << newFrontierNodes.size() << std::endl;
		state = 'e';
		return GRAB;
	}
...
void MyAI::exploreTile(bool stench, bool breeze, bool glitter, bool bump, bool scream) {
	...
	for (TileNode* node : currentNode->getNeighbors()) {
		...
		if (node->stenchCount > 1 && !killedMonster && monsterNode == nullptr) continue;
		if (node == monsterNode) continue;
