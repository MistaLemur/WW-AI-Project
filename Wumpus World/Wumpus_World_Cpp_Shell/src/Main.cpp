...
int main ( int argc, char *argv[] )
{
	...
	{
		// Parse Options
		for ( unsigned int index = 1; index < firstToken.size(); ++index )
		{
	...
	if ( folder )
	{
		...
		while ( ( ent = readdir (dir) ) != NULL )
		{
			...
			try
			{
				...
				if (score > 0) {
					wins++;
					winTime += 1000 - score;
					if (maxWinTime < 1000 - score) maxWinTime = 1000 - score;
				}
				if (score < -250) {
					deaths++;
					if (score < -1000) deathTime += -1000 - score;
					else deathTime += -0 - score;
					deathWorlds.push_back(individualWorldFile);
		...
		if ( outputFile == "" )
		{
			cout << "Number of runs: " << numOfScores << endl;
			cout << "The agent's best score: " << bestScore << endl;
			cout << "The agent's worst score: " << worstScore << endl;

			cout << "The agent's wins: " << wins << endl;
			if (wins > 0) cout << "The agent's avg win time: " << (winTime / wins) << endl;
			if (wins > 0) cout << "The agent's max win time: " << maxWinTime << endl;

			cout << "The agent's deaths: " << deaths << endl;
			if(deaths > 0) cout << "The agent's avg death time: " << (deathTime / deaths) << endl;

			cout << "The agent's average score: " << avg << endl;
			cout << "The agent's standard deviation: " << std_dev << endl;
			cout << endl;
			cout << "The agent died in the following worlds: " << endl;

			for (string file : deathWorlds) {
				cout << file << endl;
			}
		}
		else
